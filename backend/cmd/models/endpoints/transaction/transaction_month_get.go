...
func GetMonthHandler(c *fiber.Ctx) error {
	...
	result := database.Gorm.Table("transactions").
		...
		Where("transactions.owner_id = ? AND ((transactions.date <= ? AND transactions.date > ?) OR DATE(transactions.date) = DATE(?))", claims.UserId, time.Now(), time.Now().AddDate(0, -1, 0), time.Now()).
